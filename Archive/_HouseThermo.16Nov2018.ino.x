...
const float timeOffset[25] = {0,
    -15.0, -15.0, -15.0, -15.0, -10.0,   -5.0,  // 1 AM to 6 AM
      0.0,   0.0,   0.0,  -1.0,  -4.0,   -6.0,  // 7 AM to Noon
     -6.0,  -6.0,  -3.0,  -2.0,   0.0,    0.0,  // 1 PM to 6 PM
     -1.0,  -3.0,  -6.0,  -8.0, -10.0, -10.0}; // 7 PM to Midnight
...
void setup() 
{
  ...
  if(rtc[6]<18) {
    delay(500); // maybe it'll work if jolted awake by the previous call?!
    RTC.get(rtc,true);
  }
  // Set following to true to force RTC reset. ALWAYS return to false for final run, 
  // otherwise every power reset will reset the RTC back to today's date!!!
  ...
  { 
    ...
    RTC.set(DS1307_MIN,7);
    RTC.set(DS1307_HR,8);
    RTC.set(DS1307_DOW,6); // DayOfWeek (1=Sun, 7=Sat)
